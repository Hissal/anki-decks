...
5. `MemberChars` — bucket 1. Compound characters that take this row's **exact** sound (syllable + tone). Seeded from the source's curated set, then at import time each char is simplified (opencc), containment-checked (must actually contain the component — via `component_cwc.json` membership or its decomposition), and tone-classified by its **primary pypinyin reading** (matching HanziCraft's regularity degree), with a heteronym fallback so a curated member is never lost when pypinyin's primary differs from its series reading. Wrong-tone chars are demoted to bucket 2; chars that don't contain the component, or share neither its syllable nor its tone, are dropped. The component itself is stripped so the front of Card 2 is a real recognition test. If no exact-tone member survives the filtering, the whole row is dropped (bogus or archaic component — see **Simplified-only** below).
6. `SameSyllableChars` — bucket 2. Characters that contain this component AND share its syllable but DIFFER in tone (e.g. for `工:gōng`, this is `巩鞏汞銾` — same `gong` syllable but tone 3). Derived from HanziCraft's full member-char list (`scripts/cache/component_cwc.json`), simplified (opencc) and classified by each char's **primary pypinyin reading** (matching HanziCraft's "Regularity Degree Two"); each candidate must still contain the component, plus any chars demoted from bucket 1 for tone. Classifying by the primary reading (not "any heteronym reading") is what keeps chars like `们` (primary *men* neutral, secondary `men2`) in bucket 2 instead of dropping them. Chars already in MemberChars are excluded. Empty when no bucket-2 chars exist.
...
The components deck is simplified-only, like the word decks. The importer enforces this at generation time:

- **Traditional member chars** are converted to simplified (opencc); the simplified twin is kept and de-duplicated (`芳坊钫鈁枋` → `芳坊钫枋`).
- **Redundant traditional-component rows** — those whose simplified twin already exists as its own row (`華`/`华`, `韋`/`韦`, …) — are dropped (50 rows).
- **Trad-only components** with no simplified twin row are relabeled to simplified: `馬→马`, `見→见`, `門→门`.
- **Archaic / bogus components** that produce no valid simplified exact-tone member are dropped (`皀` is a non-series; `昜 巠 睪 䜌` are archaic phonetic shapes that simplification replaced with a different glyph, so they can't be taught as a simplified component). `䜌`'s series is already covered by the existing `亦` row.

Net effect: 476 → 421 rows. The dropped `巠 / 睪 / 昜` series are high-frequency but their simplified phonetic is an obscure glyph (`𢀖` / `𠬤` / `𠃓`); re-adding them as obscure-component rows is a possible future enhancement.
...
- **`scripts/import_phonetic_components.py`** — one-shot generator. Reads the rough HanziCraft-derived source notes file, optionally merges in HanziCraft enrichment, and writes the deck TSV. Not idempotent; overwrites on every run. Phase-1 cleanup includes pinyin tonemark conversion, reliability parsing, multi-line row repair, HTML-entity stripping, column-misalignment fixes, and merge-on-dedup. Phase-2 enrichment layers in Meaning (overrides mnemonics), Productivity, Frequency, and Link from the cached HanziCraft data. A **correctness/simplify pass** (pypinyin primary readings + opencc + `component_cwc.json`/decomposition) then enforces simplified-only output and the bucket-1/bucket-2 tone+containment invariants described under **Simplified-only** above — the source's curated member sets are validated and repaired, not trusted blindly (this is what catches errors like `于`'s old `余餘予馀` members, none of which contain `于`). Runtime deps for this pass and for `validate_components.py`'s deep checks: `pip install pypinyin opencc` (no dependency manifest in the repo). Rows are written in **learning-order** — a frequency-weighted sort (see `char_freq.json`).
...
- **`scripts/cache/component_cwc.json`** — per-component "chars-with-this-component" list, e.g. `{"工": ["巩", "汞", "鞏", ...]}`. Populates bucket-2 / bucket-3 reasoning on Card 2 back, and is the **authority for containment** (whether a char is actually built from the component — matches live HanziCraft; verified during the `于` audit). Generated by the same authenticated-session pass.
- **`scripts/cache/char_data.json`** — per-character pinyin keyed by Hanzi, e.g. `{"巩": {"pinyin": "gong3"}, ...}`. Covers every char that appears across all `component_cwc.json` lists (~5900 chars). **Superseded for bucket classification** by pypinyin (primary reading, heteronym fallback) — char_data is single-reading and mis-bucketed polyphones (`蔓` has màn, `轲` has kě, `盛` has chéng). Still loaded by the importer but no longer drives tone bucketing.
...
- **`scripts/cache/char_freq.json`** — per-character corpus frequency as a Zipf value (log-scaled: ~7 = 的, ~5–6 common, ~3 uncommon, <3 rare, absent ⇒ 0) keyed by Hanzi, e.g. `{"的": 7.0, "请": 5.77, …}`. ~6200 chars. Drives the **learning-order sort**: `score = Σ zipf(member) + 0.3·Σ zipf(SameSyllable) + 0.6·zipf(component)`, descending — so components whose derived characters you'll actually encounter rank first, instead of whichever happens to have the most (often archaic) members. Weights are tunable constants (`SORT_WEIGHT_*`) in the importer. Read as a static cache; no runtime frequency dependency.
- **`scripts/build_char_freq.py`** — regenerates `char_freq.json` from the `wordfreq` package's Chinese data (`pip install wordfreq`; build-time only). Run after adding new characters to the deck.
- **`scripts/validate_components.py`** — sibling of `validate.py` for the new schema. Hard errors on missing required fields, non-CJK components, digit-tone pinyin, MemberChars still containing the component, non-integer Productivity/Frequency, and duplicate Keys. Plus **deep semantic checks** (skip with `--no-deep`): every member / SameSyllable char must be simplified (opencc) and must contain the component (cwc/decomposition) — hard errors; exact-tone bucketing is checked via pypinyin heteronyms as warnings (pypinyin can miss a rare reading, so it never hard-fails).
...
8. `MemberChars` — example characters where this radical is the *semantic head*. Seeded from the curated set / seed file, then at import time **simplified-cleaned** (`finalize_members`): every char is simplified (opencc), the radical itself and its variant glyphs are stripped out, **each char's actual containment of the radical is verified against `char_decomp_hanzipy.json`** (drops cwc shape-pollution — HanziCraft's cwc wrongly lists 颈 under 川, 戴 under 矛; hanzipy decomposes them correctly and they're rejected), each kept positional variant is guaranteed ≥1 representative char, and the set is backfilled from `cwc[radical]` by frequency to a decent size (≤8). Glyph families (`_FORM_GROUPS`) let a member spelled with 巛/⺙/月 match the radical 川/攵/⺼. A few archaic radicals (黹, 飞, 黍…) head no common simplified char and end up with an empty set — they keep only their recognition card.
...
12. `MemberDecomp` — `海=氵+每|河=氵+可` per-member-char once-level decomp, generated from `char_decomp_hanzipy.json` (hanzipy / cjk-decomp, ~7500 chars — far better coverage than the HanziCraft `char_decomp.json`). Decomp glyphs are canonicalized to the deck's radical/variant form (⺙→攵) so Card 4 back can highlight the radical (and its variants) in green. Atomic members (用, 己) that can't be decomposed get no entry.
...
- **`scripts/import_kangxi_radicals.py`** — one-shot generator. Reads the seed `Radicals.txt`, parses the mixed-convention col-1, hoists parens-packed variants into Variant1/2/ReferenceVariants slots, applies `VARIANT_OVERRIDES` for radicals whose source order is wrong (水: 氵 belongs before 氺), and emits the TSV. The **R3 simplified pass** then: drops traditional variants to the Note, de-dups alternate encodings (`_VARIANT_CANON`), runs `finalize_members` (simplify + strip radical + **hanzipy containment filter** + guarantee variant representation + frequency backfill), demotes archaic radicals to `radical-rare`, and **sorts by tier then visibility** (frequency-weighted via `char_freq.json`) so common radicals come first and rare ones land at the bottom. Needs `opencc` + `char_freq.json` + `char_decomp_hanzipy.json`; build-time `wordfreq` / `hanzipy` only via the cache builders.
- **`scripts/cache/char_decomp_hanzipy.json`** — comprehensive, accurate per-character decomposition from the `hanzipy` package (cjk-decomp, ~7500 chars). The authority for member containment (does the char *actually* contain the radical) and for the `MemberDecomp` breakdown — it fixes the cwc shape-pollution the sparse HanziCraft `char_decomp.json` couldn't catch (颈 under 川, 戴 under 矛).
- **`scripts/build_char_decomp_hanzipy.py`** — regenerates that cache (`pip install hanzipy`; build-time only). Run after adding new radicals / member candidates.
