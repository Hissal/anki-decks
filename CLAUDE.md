...
5. `MemberChars` — bucket 1. Compound characters that take this row's **exact** sound (syllable + tone). Seeded from the source's curated set, then at import time each char is simplified (opencc), containment-checked (must actually contain the component — via `component_cwc.json` membership or its decomposition), and tone-classified via pypinyin heteronyms. Wrong-tone chars are demoted to bucket 2; chars that don't contain the component, or share neither its syllable nor its tone, are dropped. The component itself is stripped so the front of Card 2 is a real recognition test. If no exact-tone member survives the filtering, the whole row is dropped (bogus or archaic component — see **Simplified-only** below).
6. `SameSyllableChars` — bucket 2. Characters that contain this component AND share its syllable but DIFFER in tone (e.g. for `工:gōng`, this is `巩鞏汞銾` — same `gong` syllable but tone 3). Derived from HanziCraft's full member-char list (`scripts/cache/component_cwc.json`), simplified (opencc) and read via **pypinyin heteronyms** (multi-reading); each candidate must still contain the component, plus any chars demoted from bucket 1 for tone. Chars already in MemberChars are excluded. Empty when no bucket-2 chars exist.
